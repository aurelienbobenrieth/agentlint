...
- **The review server is loopback only.** `agentlint review` binds to `127.0.0.1`, requires a per-session token in the URL, and serves the bundled SPA. It opens files in an editor only through an allowlisted launcher, after resolving and validating the path inside the repository.
