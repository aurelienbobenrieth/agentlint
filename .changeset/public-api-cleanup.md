...
- `@aurelienbbn/agentlint/testing` now exports the promise helpers `testRuleFixtures`, `testRuleOnSource`, `testRuleOnSources`, `testRuleOnChange`, plus `normalizeChangeFixture` and the `FixtureReport` and `FixtureFailure` types. They are no longer exported from the package root.
