...
try {
  writeFileSync(join(root, "package.json"), encodeJson({ name: "agentlint-package-smoke", private: true }));
  run({
    command: npm,
    args: [...npmPrefix, "install", "--ignore-scripts", "--no-audit", "--no-fund", tarball, "typescript@7.0.2"],
  });
...
const rule = defineRule({ lifecycle: "change", standard: { id: "typed", revision: 1, title: "Typed", guidance: "Review" },
 detector: { id: "typed", version: 1, detect({ options }) { if (options.limit < 0) throw new Error("limit"); } },
 binding: { id: "typed", authority: "agent", options: { limit: 5 } } });
defineConfig({ rules: [rule] });
void testRuleOnChange({ rule, fixture: { before: {}, after: {} } });
...
`,
  ...
  run({ command: process.execPath, args: [bin, "init"] });
  writeFileSync(
...
`,
  ...
  run({ command: process.execPath, args: [bin, "rules", "test"] });
  run({ command: process.execPath, args: [bin, "check", "--all"], expected: 1 });
  const consumerRequire = createRequire(join(root, "package.json"));
  const { Schema: ConsumerSchema } = await import(pathToFileURL(consumerRequire.resolve("effect")).href);
  const { NextResult } = await import(
    pathToFileURL(join(root, "node_modules", "@aurelienbbn", "agentlint", "dist", "contract.mjs")).href
  );
  const decodeNext = ConsumerSchema.decodeUnknownSync(ConsumerSchema.fromJsonString(NextResult));
  const handoff = decodeNext(run({ command: process.execPath, args: [bin, "next", "--format", "json"], expected: 1 }));
  if (
    handoff.version !== 1 ||
    handoff.remaining !== 1 ||
  ...
  run({ command: process.execPath, args: [bin, "explain", "1"] });
  run({
    command: process.execPath,
    args: [bin, "accept", "1", "--reason", "The call runs inside the verified test sandbox."],
  });
  run({ command: process.execPath, args: [bin, "check", "--all"] });

  const cleared = decodeNext(run({ command: process.execPath, args: [bin, "next", "--format", "json"] }));
  ...
  writeFileSync(
    ...
    encodeJson({ name: "agentlint-smoke-presets", type: "module", exports: "./index.mjs" }),
  );
  writeFileSync(
  ...
  run({
    command: process.execPath,
    args: [bin, "init", "--preset", "agentlint-smoke-presets#starterPreset"],
    expected: 0,
    cwd: presetConsumer,
  });
  run({ command: process.execPath, args: [bin, "rules", "test"], expected: 0, cwd: presetConsumer });
  run({ command: process.execPath, args: [bin, "next", "--format", "json"], expected: 1, cwd: presetConsumer });
