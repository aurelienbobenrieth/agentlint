...
function addGuidance({ lines, rule }: { readonly lines: string[]; readonly rule: AgentlintRule }): void {
...
export const explainHandler = Effect.fn("explainHandler")(function* (command: ExplainCommand) {
  ...
  const acceptanceStore = yield* AcceptanceStore;
  ...
  const snapshot = yield* acceptanceStore.read();
  const state = lookupAcceptance({ acceptances: snapshot, finding }) ? "accepted" : "unresolved";
  ...
  if (lineage) {
    ...
    lines.push(...invalidationReasons({ prior: lineage, current: finding }));
