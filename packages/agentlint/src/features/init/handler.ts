...
export const initHandler = Effect.fn("initHandler")(function* (command: InitCommand) {
    ...
    const config =
      presets.length === 0
        ...
        : [
            ...
            ...A.map(
              presets,
              (preset, index) => `import { ${preset.name} as preset${index} } from ${encodeJson(preset.module)}`,
            ),
            "",
            "export default defineConfig({",
            `  extends: [${A.map(presets, (_, index) => `preset${index}`).join(", ")}],`,
            '  ignores: ["dist/**", "coverage/**"],',
            "})",
            "",
          ].join("\n");
  ...
  const currentEntries = currentIgnore.split(/\r?\n/);
  const missingEntries = A.filter(ephemeralEntries, (entry) => !A.contains(currentEntries, entry));
  ...
  return new InitResult({
    ...
    message: [
      ...
      ...(presets.length && created
        ? [
            `Install the selected packages: pnpm add -D ${[...new Set(A.map(presets, (preset) => preset.module))].join(" ")}`,
